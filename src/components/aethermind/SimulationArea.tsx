...
import { getStrategyExplanation, getPersonalizedSuggestions, getOkxMarketConditions } from '@/lib/actions'; // Updated import
...
export function SimulationArea({ userTokenHoldingsString, onSimulationComplete }: SimulationAreaProps) {
  ...
  const runSimulation = async () => {
    ...
    startSimulationTransition(async () => {
      const marketConditions = await getOkxMarketConditions(); // Use updated function
      ...
      if (selectedStrategy.id === 'yield-farming') {
        result.estimatedAPY = `${(Math.random() * 15 + 5).toFixed(2)}% (AI Projected)`; // Mock APY
